...
import { cn } from "@/lib/utils"; // Assumindo que você tem o cn

type SharkSwimProps = {
  width?: number | string; // aceita número (ex: 200) ou string (ex: "50%")
  className?: string; // para classes Tailwind como text-foreground ou cor fixas
  color?: "white" | "black" | "auto" | string; // 'white', 'black', 'auto' (dinâmico), ou custom CSS
};
...
export function SharkSwim({
  ...
  // 🔑 LÓGICA DE COR FINAL:
  // 1. Se color é "auto", usamos "currentColor" para herdar do Tailwind (solução dinâmica).
  // 2. Caso contrário, usamos a prop color como fill.
  const finalFillColor = color === "auto" ? "currentColor" : color;

  // 🚨 Para evitar o problema de cache do React, removemos a prop 'fill' do SVG principal
  // e aplicamos o fill="currentColor" ou fill={finalFillColor} em CADA path.
      ...
      >
        ...
        >
          <path
            ...
            fill={finalFillColor} // 🔑 APLICAÇÃO DINÂMICA
          />
          <path
            fillRule="evenodd"
            clipRule="evenodd"
